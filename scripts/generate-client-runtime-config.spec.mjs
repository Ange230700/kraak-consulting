...
test('le runtime client expose aussi les URLs publiques de marque et contact quand elles sont définies', () => {
...
test('le runtime client peut lire les variables locales depuis apps/client/.env quand le fichier existe', () => {
  const tempRoot = mkdtempSync(
    path.join(os.tmpdir(), 'kraak-client-runtime-config-'),
  );
...
test('le runtime client peut lire les variables staging depuis process.env quand aucun fichier .env n est disponible', () => {
  const tempRoot = mkdtempSync(
    path.join(os.tmpdir(), 'kraak-client-runtime-config-'),
  );
...
test('le runtime client lit les variables production depuis .env.prod quand le fichier existe', () => {
  const tempRoot = mkdtempSync(
    path.join(os.tmpdir(), 'kraak-client-runtime-config-'),
  );
...
test('le runtime client priorise les variables du fichier .env sur celles de process.env', () => {
  const tempRoot = mkdtempSync(
    path.join(os.tmpdir(), 'kraak-client-runtime-config-'),
  );
...
test('le runtime client supporte la syntaxe export, les guillemets et les sauts de ligne échappés depuis le fichier env', () => {
  const tempRoot = mkdtempSync(
    path.join(os.tmpdir(), 'kraak-client-runtime-config-'),
  );
...
test('le runtime client expose enableParticipantArea quand CLIENT_FEATURE_PARTICIPANT_AREA vaut "true"', () => {
  const tempRoot = mkdtempSync(
    path.join(os.tmpdir(), 'kraak-client-runtime-config-'),
  );
...
test('le runtime client expose enableParticipantArea=false quand la variable vaut une valeur autre que "true"', () => {
  const tempRoot = mkdtempSync(
    path.join(os.tmpdir(), 'kraak-client-runtime-config-'),
  );
...
test('la serialisation du runtime produit un script executable qui fige la configuration', () => {
