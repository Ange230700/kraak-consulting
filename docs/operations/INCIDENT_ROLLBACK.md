...
# Incident, rollback et checklist de lancement

## Table des matières

- [Incident, rollback et checklist de lancement](#incident-rollback-et-checklist-de-lancement)
  - [Objectif](#objectif)
  - [Dépendances](#dependances)
  - [Portée](#portee)
  - [Flux synthétique](#flux-synthetique)
  - [1. Incident Response Playbook](#1-incident-response-playbook)
    - [1.1 Détection d'incident](#11-detection-dincident)
      - [Signaux de monitorage](#signaux-de-monitorage)
      - [Escalade urgente](#escalade-urgente)
    - [1.2 Diagnostic d'incident](#12-diagnostic-dincident)
      - [Phase 1 : Isoler le domaine](#phase-1-isoler-le-domaine)
      - [Phase 2 : Collecte de données](#phase-2-collecte-de-donnees)
        - [Web (Render)](#web-render)
        - [API (Render)](#api-render)
        - [Supabase](#supabase)
    - [1.3 Mitigation immédiate](#13-mitigation-immediate)
      - [Scénario A : Web indisponible, API OK](#scenario-a-web-indisponible-api-ok)
      - [Scénario B : API indisponible, Web OK (error 500/timeout)](#scenario-b-api-indisponible-web-ok-error-500timeout)
      - [Scénario C : Lenteur généralisée (API/Web lents)](#scenario-c-lenteur-generalisee-apiweb-lents)
      - [Scénario D : Erreurs spécifiques par route (ex: /programs en 500, / en 200)](#scenario-d-erreurs-specifiques-par-route-ex-programs-en-500-en-200)
    - [1.4 Post-incident](#14-post-incident)
  - [2. Rollback Procedures](#2-rollback-procedures)
    - [2.1 Rollback Web (Render)](#21-rollback-web-render)
      - [Méthode A : Redeploy depuis l'UI Render (rapide)](#methode-a-redeploy-depuis-lui-render-rapide)
      - [Méthode B : Redeploy depuis Git (fiable)](#methode-b-redeploy-depuis-git-fiable)
      - [Méthode C : Rollback complet depuis une étiquette (sûr)](#methode-c-rollback-complet-depuis-une-etiquette-sur)
    - [2.2 Rollback API (Render)](#22-rollback-api-render)
      - [Méthode A : Redeploy un commit précédent depuis Render UI](#methode-a-redeploy-un-commit-precedent-depuis-render-ui)
      - [Méthode B : Redeploy depuis Git (tracé)](#methode-b-redeploy-depuis-git-trace)
      - [Méthode C : Redémarrer sans revert (si c'est un problème temporaire)](#methode-c-redemarrer-sans-revert-si-cest-un-probleme-temporaire)
    - [2.3 Coordonner un rollback multi-service](#23-coordonner-un-rollback-multi-service)
    - [2.4 Prévention de corruption lors de rollback](#24-prevention-de-corruption-lors-de-rollback)
      - [Données d'application](#donnees-dapplication)
      - [Cache client](#cache-client)
  - [3. Pilot Launch Checklist](#3-pilot-launch-checklist)
    - [3.1 Pre-launch Validation (24-48h avant pilot)](#31-pre-launch-validation-24-48h-avant-pilot)
      - [Infrastructure & Déploiement](#infrastructure-deploiement)
      - [Observabilité & Alertes](#observabilite-alertes)
      - [Tests Pré-Pilot](#tests-pre-pilot)
      - [Parcours Utilisateur Critiques](#parcours-utilisateur-critiques)
      - [Sécurité & Conformité](#securite-conformite)
      - [Données & Base de Données](#donnees-base-de-donnees)
      - [Documentation & Support](#documentation-support)
    - [3.2 Jour du Launch (Go/No-Go)](#32-jour-du-launch-gono-go)
      - [Morning Check (2h avant pilot)](#morning-check-2h-avant-pilot)
      - [Go/No-Go Decision](#gono-go-decision)
    - [3.3 Launch Window](#33-launch-window)
    - [3.4 Post-Launch Monitoring](#34-post-launch-monitoring)
  - [4. Communication & Escalade](#4-communication-escalade)
    - [4.1 Canaux d'alerte](#41-canaux-dalerte)
    - [4.2 Escalade](#42-escalade)
  - [5. Annexe : Checklist Rapide de Poche](#5-annexe-checklist-rapide-de-poche)
  - [6. Historique et Mises à Jour](#6-historique-et-mises-a-jour)
  - [Ressources Externes](#ressources-externes)

> Origine historique : DEP-06, 2026-04-30. Ce fichier est désormais le runbook
> actif pour incident, rollback et checklist de lancement.
...
Action 2 : Redéployer le dernier artefact validé
- Utiliser Render UI > Deployments > Redeploy
- Ou relancer le workflow de release sur un tag SemVer déjà validé
...
3. Créer une branche rollback : `git switch -c fix/rollback-<yyyymmdd-hhmm>`
...
git switch -c fix/rollback-from-<tag> <tag>
...
1. Identifier le dernier commit stable : `git log --oneline -10`
2. Créer une branche rollback : `git switch -c fix/api-rollback-<yyyymmdd-hhmm>`
...
- [ ] Regression suite exécutée (QAT-05)
  - Voir: `docs/archive/pilot-2026-04/QAT-05_REGRESSION_2026-04-30.md`
...
- [ ] Migrations Supabase appliquées
  ...
  - `docs/architecture/DATA_MODEL.md` synchronisé avec le schéma réel
