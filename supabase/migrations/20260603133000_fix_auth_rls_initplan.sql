...
CREATE POLICY participant_select_access ON participant
  FOR SELECT
  USING (is_admin() OR user_id = (select auth.uid()));

CREATE POLICY participant_update_access ON participant
  FOR UPDATE
  USING (is_admin() OR user_id = (select auth.uid()))
...
CREATE POLICY session_select_access ON session
  FOR SELECT
  USING (
    is_admin()
...
CREATE POLICY support_request_select_access ON support_request
  FOR SELECT
  USING (is_admin() OR user_id = (select auth.uid()));

CREATE POLICY support_request_insert_access ON support_request
  FOR INSERT
  WITH CHECK (is_admin() OR user_id = (select auth.uid()));

CREATE POLICY support_request_update_access ON support_request
  FOR UPDATE
  USING (is_admin() OR user_id = (select auth.uid()))
  WITH CHECK (is_admin() OR user_id = (select auth.uid()));
