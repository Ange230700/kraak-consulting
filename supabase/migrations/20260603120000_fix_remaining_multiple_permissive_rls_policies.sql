...
CREATE POLICY app_user_select_access ON app_user
  FOR SELECT
  USING (is_admin() OR id = auth.uid());

CREATE POLICY app_user_insert_access ON app_user
  FOR INSERT
  WITH CHECK (is_admin() OR id = auth.uid());

CREATE POLICY app_user_update_access ON app_user
  FOR UPDATE
  USING (is_admin() OR id = auth.uid())
  WITH CHECK (is_admin() OR id = auth.uid());
...
CREATE POLICY announcement_select_access ON announcement
  FOR SELECT
  USING (
    is_admin()
    OR (
...
CREATE POLICY announcement_insert_admin ON announcement
  FOR INSERT
  WITH CHECK (is_admin());

CREATE POLICY announcement_update_admin ON announcement
  FOR UPDATE
  USING (is_admin())
  WITH CHECK (is_admin());

CREATE POLICY announcement_delete_admin ON announcement
  FOR DELETE
  USING (is_admin());
...
CREATE POLICY cohort_select_access ON cohort
  FOR SELECT
  USING (
    is_admin()
    OR EXISTS (
...
CREATE POLICY cohort_delete_admin ON cohort
  FOR DELETE
  USING (is_admin());
...
CREATE POLICY enrollment_select_access ON enrollment
  FOR SELECT
  USING (
    is_admin()
    OR EXISTS (
...
CREATE POLICY notification_select_access ON notification
  FOR SELECT
  USING (is_admin() OR user_id = auth.uid());

CREATE POLICY notification_insert_admin ON notification
  FOR INSERT
  WITH CHECK (is_admin());

CREATE POLICY notification_update_access ON notification
  FOR UPDATE
  USING (is_admin() OR user_id = auth.uid())
  WITH CHECK (is_admin() OR user_id = auth.uid());

CREATE POLICY notification_delete_admin ON notification
  FOR DELETE
  USING (is_admin());
...
CREATE POLICY program_select_access ON program
  FOR SELECT
  USING (
    is_admin()
...
CREATE POLICY program_insert_admin ON program
  FOR INSERT
  WITH CHECK (is_admin());

CREATE POLICY program_update_admin ON program
  FOR UPDATE
  USING (is_admin())
  WITH CHECK (is_admin());

CREATE POLICY program_delete_admin ON program
  FOR DELETE
  USING (is_admin());
...
CREATE POLICY resource_select_access ON resource
  FOR SELECT
  USING (
    is_admin()
    OR EXISTS (
