...
CREATE POLICY participant_insert_admin ON participant
  FOR INSERT
  WITH CHECK (is_admin());

CREATE POLICY participant_delete_admin ON participant
  FOR DELETE
  USING (is_admin());
...
CREATE POLICY support_request_delete_admin ON support_request
  FOR DELETE
  USING (is_admin());